...
Keyball44 是一块 44 键的无线分体键盘：nice!nano 主控、内置轨迹球，屏幕可以选
...
`leftball` = 轨迹球挪到左半，`dongle` = 用接收器当主机。
...
| `dongle` | 右半 | OLED | **XIAO BLE 接收器** | 用 `keyball44_dongle` + `prospector_adapter` 做中央，左右半变外设 |
...
基線分支（`main` / `niceview` / `leftball*` / `dongle*`）仍然是原来的 ZMK v0.3 +
tangbonze 驱动配置，没有改动。
...
* 默认上限：8 个宏（每个最大 256 字节）、8 个组合键（每个最多 16 个键位），要更多就
  改 `keyball44_right.conf` 里的 `ZMK_RUNTIME_MACRO_*` / `ZMK_RUNTIME_COMBO_*`。
...
| 产物 | 用途 |
| --- | --- |
| `keyball44_right.uf2`（dya / dya-nv 的产物名） | 右半 = **主手（中央）**：接 USB / 蓝牙，轨迹球在这一半 |
| `keyball44_left.uf2` | 左半 = 副手（外设） |
| `keyball44_reset.uf2` | 清空已保存的设置（键位改动、配对信息等），从固件默认值重新开始 |
...
Keyball44 is a 44-key wireless split keyboard: nice!nano controller, built-in
...
nice!view display, `leftball` = trackball moved to the left half, `dongle` = a receiver
acts as the central.
...
| `dongle` | right | OLED | **XIAO BLE receiver** | `keyball44_dongle` + `prospector_adapter` as the central, both halves as peripherals |
...
any branch. The base-line branches (`main` / `niceview` / `leftball*` / `dongle*`) keep
the original ZMK v0.3 + tangbonze driver configuration.
...
* Defaults: 8 macros (256 bytes each) and 8 combos (16 positions each) — tune them with
  `ZMK_RUNTIME_MACRO_*` / `ZMK_RUNTIME_COMBO_*` in `keyball44_right.conf`.
...
| Artifact | Purpose |
| --- | --- |
| `keyball44_right.uf2` (dya / dya-nv artifact name) | Right half = **main hand (central)**: USB/BLE to the host, and the trackball |
| `keyball44_left.uf2` | Left half = peripheral |
| `keyball44_reset.uf2` | Wipe stored settings (keymap edits, pairings, …) and start from firmware defaults |
...
* Battery history writes to flash periodically; turn `CONFIG_ZMK_BATTERY_HISTORY*` off in
  `keyball44_right.conf` if you don't need it.
* `dya` / `dya-nv` have moved from the ZMK v0.3 line to `main+dya` (ZMK main /
  ...
  **flash `keyball44_reset.uf2` first when switching between the two lines**.
